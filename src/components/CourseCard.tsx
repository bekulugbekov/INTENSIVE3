...
import { Clock, User, BookOpen, Target, X, Info, ArrowRight, GraduationCap, Languages, Award, Zap, Heart, Crown, MessageCircle } from 'lucide-react';
...
export const CourseCard: React.FC<CourseCardProps> = ({ 
  ...
  const getProgress = () => {
    const seed = (teacher + level).length;
  ...
  return (
    <>
      <motion.div 
        ...
        className={`group relative bg-white/40 dark:bg-[#242830]/40 backdrop-blur-xl border border-white/50 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-500 ease-out flex flex-col h-full ${isLarge ? 'lg:flex-row' : ''}`}
      >
        {isLarge && (
          <div className="lg:w-2/5 relative overflow-hidden">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-transparent to-white/40 dark:to-[#242830]/40 hidden lg:block" />
        ...
        <div className={`p-8 md:p-10 flex flex-col flex-1 ${isLarge ? 'lg:p-12' : ''}`}>
          ...
          <div className="flex-grow">
            ...
            >
              <div className="relative">
                <div className={`w-14 h-14 rounded-2xl bg-neutral-100 dark:bg-white/5 flex items-center justify-center overflow-hidden border border-white/50 dark:border-white/10 group-hover/instructor:border-current transition-colors duration-300 ${teacherStyle.color}`}>
                  {instructor?.photo ? (
                    <img src={instructor.photo} alt={teacher} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
            ...
            {isLarge && (
              <p className="text-neutral-600 dark:text-[#A0AEC0] mb-8 line-clamp-2 text-lg italic font-serif">
                "{curriculum}"
              </p>
            )}
            ...
            <div className={`grid gap-4 mb-8 ${isLarge ? 'sm:grid-cols-2' : 'grid-cols-1'}`}>
              {schedule.map((time, idx) => (
                <div key={idx} className="flex items-center gap-4 text-neutral-600 dark:text-[#A0AEC0] group/time">
                  <div className={`w-8 h-8 rounded-xl bg-neutral-100/50 dark:bg-white/5 flex items-center justify-center group-hover/time:${teacherStyle.bg} transition-colors duration-300`}>
          ...
          <button 
            ...
            className="relative w-full py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] overflow-hidden group/btn transition-all duration-300 mt-auto"
          >
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6 pr-12 sm:pr-0">
                  ...
                  {instructor && (
                    <button 
                      ...
                      className="flex items-center justify-center gap-2 px-5 py-2.5 bg-neutral-100 dark:bg-[#1A1D23] text-neutral-900 dark:text-[#EAEAEA] text-sm font-bold rounded-xl hover:bg-neutral-900 hover:text-white dark:hover:bg-[#EAEAEA] dark:hover:text-[#1A1D23] transition-all shadow-sm w-fit"
                    >
                      <User size={16} />
                      {t('view_profile')}
                    </button>
